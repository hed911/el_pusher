...
module ElPusher
  ...
  # @example Sinatra
  #   post '/webhooks' do
  #     webhook = ElPusher::WebHook.new(request)
  #     if webhook.valid?
  ...
  class WebHook
    ...
    #
    def initialize(request, client = ElPusher)
      @client = client
    ...
    def valid?(extra_tokens = nil)
      ...
      ElPusher.logger.warn "Received webhook with unknown key: #{key}"
      return false
    end
    ...
    def check_signature(secret)
      ...
      if @signature == expected
        ...
        ElPusher.logger.warn "Received WebHook with invalid signature: got #{@signature}, expected #{expected}"
        return false
      end
    end
  end
