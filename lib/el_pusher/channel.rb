...
module ElPusher
  ...
  class Channel
    ...
    def initialize(_, name, client = ElPusher)
      if ElPusher::Channel::INVALID_CHANNEL_REGEX.match(name)
        raise ElPusher::Error, "Illegal channel name '#{name}'"
      elsif name.length > 200
        raise ElPusher::Error, "Channel name too long (limit 164 characters) '#{name}'"
      end
      @name = name
      @client = client
    end
    ...
    # switch to ElPusher.trigger_async or ElPusher::Client#trigger_async instead
    #
    # @param (see #trigger!)
    ...
    # @raise [ElPusher::Error] unless the eventmachine reactor is running. You
    ...
    #
    # [Deprecated] This method will be removed in a future gem version. Please
    # switch to ElPusher.trigger or ElPusher::Client#trigger instead
    #
    # @example
    #   begin
    #     ElPusher['my-channel'].trigger!('an_event', {:some => 'data'})
    #   rescue ElPusher::Error => e
    ...
    #
    # @raise [ElPusher::Error] on invalid Pusher response - see the error message for more details
    # @raise [ElPusher::HTTPError] on any error raised inside http client - the original error is available in the original_error attribute
    #
    ...
    def trigger(event_name, data, socket_id = nil)
      ...
      ElPusher.logger.error("#{e.message} (#{e.class})")
      ElPusher.logger.debug(e.backtrace.join("\n"))
    end
    ...
    # @raise [ElPusher::Error] on invalid Pusher response - see the error message for more details
    # @raise [ElPusher::HTTPError] on any error raised inside Net::HTTP - the original error is available in the original_error attribute
    #
    def users(params = {})
    ...
    # @return [String]
    #
    # @raise [ElPusher::Error] if socket_id or custom_string invalid
    #
    def authentication_string(socket_id, custom_string = nil)
      ...
      ElPusher.logger.debug "Signing #{string_to_sign}"
    ...
    #   render :json => ElPusher['private-my_channel'].authenticate(params[:socket_id])
    #
    ...
    # @return [Hash]
    #
    # @raise [ElPusher::Error] if socket_id or custom_data is invalid
    #
    ...
    def validate_socket_id(socket_id)
      unless socket_id && /\A\d+\.\d+\z/.match(socket_id)
        raise ElPusher::Error, "Invalid socket ID #{socket_id.inspect}"
      end
    end
  end
end
