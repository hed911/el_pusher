...
module ElPusher
  class Client
    ...
    #
    # @example
    #   ElPusher.url = http://KEY:SECRET@api.pusherapp.com/apps/APP_ID
    #
    def url=(url)
      uri = URI.parse(url)
      @scheme = uri.scheme
    ...
    # @example
    #   begin
    #     ElPusher.get('/channels', filter_by_prefix: 'private-')
    ...
    #
    # @raise [ElPusher::Error] Unsuccessful response - see the error message
    # @raise [ElPusher::HTTPError] Error raised inside http client. The original error is wrapped in error.original_error
    #
    def get(path, params = {})
    ...
    #   ElPusher['my-channel']
    # @return [Channel]
    # @raise [ElPusher::Error] if the channel name is invalid.
    ...
    #   render :json => ElPusher.authenticate('private-my_channel', params[:socket_id])
    #
    # @example Presence channels
    #   render :json => ElPusher.authenticate('presence-my_channel', params[:socket_id], {
    ...
    # @return [Hash]
    #
    # @raise [ElPusher::Error] if channel_name or socket_id are invalid
    #
    ...
    def trigger_params(channels, event_name, data, params)
      ...
      raise ElPusher::Error, "Too many channels (#{channels.length}), max 10" if channels.length > 10
